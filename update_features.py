...
def process_file(filepath):
    ...
    # The features swiper html to replace:
    start_str = '<div class="relative group px-0 md:px-16 pb-8">\n                    <div class="swiper features-swiper relative pt-6 pb-16">\n                      <div class="swiper-wrapper">'
    ...
    if start_str in content and end_str in content:
        # We need to replace everything between start_str and end_str, including them
        prefix, rest = content.split(start_str, 1)
        _, suffix = rest.split(end_str, 1)
        
        new_content = prefix + new_features + suffix
        
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(new_content)
        print(f"Success updating {filepath}!")
    else:
        print(f"Could not find exact strings in {filepath}")
